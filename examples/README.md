...
This directory contains example scripts demonstrating different use cases for the CLOC History Analyzer.
...
```bash
node ../analyze.mjs https://github.com/yourusername/yourrepo ./my-analysis --force-full
```
...
for repo in "${REPOS[@]}"; do
  name=$(basename "$repo")
  echo "Updating $name..."
  node ../analyze.mjs "$repo" "./weekly-metrics/$name"
done
```
...
jobs:
  update-metrics:
    ...
    steps:
      ...
      - name: Update metrics
        run: |
          node analyze.mjs https://github.com/${{ github.repository }} ./metrics
...
metrics:
  ...
  script:
    ...
    - node analyze.mjs https://gitlab.com/$CI_PROJECT_PATH ./metrics
    - cp -r ./metrics public/
  artifacts:
    paths:
      - public
...
- [Incremental Updates Guide](../docs/INCREMENTAL.md)
